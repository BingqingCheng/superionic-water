...
cp equal-X-sl-P-$p.data plumed.dat single-npt.sh ../${prefix}-P-$p-T-$t-R-$r

# check if cubic
cubic=$(grep "$p $t" boxsize-${prefix}.dat | awk -v p=$p '$1==p{cub=(($3-$5)**2.+($3-$7)**2.+($5-$7)**2.)**(1./2.); if(cub > 0.8 ){print 1} else {print 0}}')

if [ $cubic == 0 ]; then

	#echo $p $t "# checked the box is cubic"

b=$(grep "$p $t" boxsize-${prefix}.dat | awk -v p=$p '$1==p{print 1.5*($3*$5*$7)**(1./3.)}')
echo $p $t "boxisize" $b
